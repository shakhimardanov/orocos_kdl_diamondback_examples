...
int main()
{
    ...
    chainLWR.addSegment(Segment(Joint(Joint::RotZ),
                  Frame::DH_Craig1989(0.1, 1.5707963, 0.0, 0.0),
    ...
    double Kp[3], Kp_b[3];
    Kp_b[0] = 1.015345/(timeToSettle*timeToSettle);
    Kp_b[1] = 1.015345/(timeToSettle*timeToSettle);
    Kp_b[2] = 1.015345/(timeToSettle*timeToSettle);

    Kp[0] = 270.15345/(timeToSettle*timeToSettle); // 1.345 for x const only  with timeToSettle= 0.015 and q1 is controlled
    Kp[1] = 270.232/(timeToSettle*timeToSettle);
    Kp[2] = 680.232/(timeToSettle*timeToSettle);
    
    double Kv[3], Kv_b[3];
    Kv_b[0] = 0.55989*ksi[1]/timeToSettle;
    Kv_b[1] = 0.55989*ksi[1]/timeToSettle;
    Kv_b[2] = 0.55989*ksi[1]/timeToSettle;
    ...
    double b1_x = 0.0;
    double b2_x = ((-0.465823 - b0_x)*3.0 / (simulationTime * simulationTime)); 
    double b3_x = -((-0.465823 - b0_x)*2.0 / (simulationTime * simulationTime * simulationTime));
    ...
    double b1_z = 0.0;
    double b2_z = ((0.544715 - b0_z)*3.0 / (simulationTime * simulationTime)); 
    double b3_z = -((0.544715- b0_z)*2.0 / (simulationTime * simulationTime * simulationTime));
    ...
    for (double t = 0.0; t <= simulationTime; t = t + timeDelta)
    {
        ...
        // betha(0) = alpha(0, 0)*(K * cartXDotDot[2][numberOfLinks-1].vel[1] + (Kv_b[0]) * cartXDot[2][numberOfLinks-1].vel[1] + (Kp_b[0]) * cartX[2][numberOfLinks-1].p[1]+ Ki[1] * cartX[3][numberOfLinks-1].p[1]); 
        ...
        #ifdef FEXT_CONTROL_WITH_CONSTRAINT
            externalNetForces[numberOfLinks-1].force[0] = ((Kv_b[0]) * cartXDot[2][numberOfLinks-1].vel[0] + (Kp_b[0]) * cartX[2][numberOfLinks-1].p[0] );
            externalNetForces[numberOfLinks-1].force[1] = ((Kv_b[1]) * cartXDot[2][numberOfLinks-1].vel[1] + (Kp_b[1]) * cartX[2][numberOfLinks-1].p[1] ); 
            externalNetForces[numberOfLinks-1].force[2] = ((Kv_b[2]) * cartXDot[2][numberOfLinks-1].vel[2] + (Kp_b[2]) * cartX[2][numberOfLinks-1].p[2] ); 
        #endif//~FEXT_CONTROL_WITH_CONSTRAINT
